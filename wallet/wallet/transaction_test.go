...
func init() {
	...
	daemon.InitClient(cfg, "v1.0", log.Root())
...
func TestCheckDest(t *testing.T) {
	...
	transTests["TestCheckDest"] = []TransTest{
		...
		},
		{
			...
			output: fmt.Sprintf("%x", big.NewInt(0).SetBytes(append(big.NewInt(2*1e11).Bytes(), mockWallet.estimateUtxoTxFee().Bytes()...))),
			error:  "",
		},
		{
			val: stest{ // t->t
			...
			output: fmt.Sprintf("%x", big.NewInt(0).SetBytes(append(big.NewInt(2*1e11).Bytes(), mockWallet.estimateTxFee(big.NewInt(2*1e11)).Bytes()...))),
			error:  "",
		},
		{
			val: stest{ // u->t token
				...
				mode:    UTXOInputMode,
				tokenID: common.BigToAddress(big.NewInt(1)),
			},
			output: fmt.Sprintf("%x", big.NewInt(0).SetBytes(append(big.NewInt(2*1e11).Bytes(), mockWallet.estimateUtxoTxFee().Bytes()...))),
			error:  "",
		},
		{
			...
			output: fmt.Sprintf("%x", big.NewInt(0).SetBytes(append(big.NewInt(2*1e11).Bytes(), mockWallet.estimateTxFee(big.NewInt(0)).Bytes()...))),
			error:  "",
		},
	}
	runTransTests(t, "TestCheckDest", func(val interface{}) ([]byte, error) {
		st := val.(stest)
		amount, fee, _, err := mockWallet.checkDest(st.dests, st.tokenID, st.mode)
		res := big.NewInt(0).SetBytes(append(amount.Bytes(), fee.Bytes()...)).Bytes()
		return res, err
	})
}
