...
func (wallet *Wallet) CreateAinTransaction(from common.Address, passwd string, nonce uint64, dests []types.DestEntry,
	...
	tx, txKey, err := types.NewAinTransaction(source, dests, tokenID, totalFee, nil)
	if err != nil {
	...
	for 0 != len(dests) || addingFee {
		...
		if tryTx {
			...
			if inAmount.Cmp(realNeedMoney) < 0 && !needDecFee { // token transfer tx won't reach this because tryTx suppose inAmount >= outAmount
				...
				utxoTrans, utxoInEphs, mKeys, txKey, err := types.NewUinTransaction(wallet.currAccount.account.GetKeys(), wallet.currAccount.account.KeyIndex, selectSources, paidDests, tokenID, refundAddr, totalFee, extra)
				if err != nil {
